...
class CustomerPreparationTaskDetailEnricher
{
    public function enrichFor(User $user, bool $includeManualOverrides = false): void
    {
        $manualDetails = $includeManualOverrides ? $this->manualDetails() : [];
        ...
        CustomerPreparationTask::query()
            ...
            ->with('section')
            ->orderBy('id')
            ->each(function (CustomerPreparationTask $task) use ($user, $manualTitles, $manualDetails, $includeManualOverrides): void {
                if ($includeManualOverrides && in_array($task->title, $manualTitles, true)) {
                    $this->applyManualDetail($user->id, $task, $manualDetails[$task->title]);

                    return;
                }

                if ($task->subTasks()->exists() || filled($task->description)) {
                    return;
                }
